...
class BaseScatLayer(Layer):
    """
    Do I have to implement this ?
    """
    def __init__(self, incoming, num_filters, filter_size, stride=1, pad=0,
                 untie_biases=False,
                 W=init.scatzeros(), b=init.Constant(0.),
                 nonlinearity=nonlinearities.modulus, flip_filters=True,
                 n=None, **kwargs):
...
class Conv1DLayer(BaseConvLayer):
    ...
    **kwargs
    ...
    def convolve(self, input, **kwargs):
        ...
        return conved
class Scat2DLayer(BaseScatLayer):
    """
    My implementation of scattering transform
    zero padded to ensure linear convolution.
    stride = (1,1)
    scattering transform params
    """
    def __init__(self, incoming, num_filters, filter_size, stride=(1, 1),
                 pad=0, untie_biases=False,
                 W=init.zeros(), b=init.Constant(0.),
                 nonlinearity=nonlinearities.rectify, flip_filters=True,
